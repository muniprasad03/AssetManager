...
appRoot.controller('costIncidentsReportController', [
    ...
    function ($scope, $q, $state, $stateParams, $location, $filter, $http, $uibModal, ReportService, UserService, NotificationService, SharedModel, AppSettings) {
        ...
        $scope.getAll = function () {
            var date = new Date();
            $scope.vm.fromDate = new Date(new Date(date.getFullYear()-1, date.getMonth() -2,1).setHours(23, 59, 59, 999));
            $scope.vm.toDate = new Date(new Date(date.getFullYear() -1, date.getMonth(),1).setHours(23, 59, 59, 999));
            $scope.vm.fromDate2 = new Date(new Date(date.getFullYear(), date.getMonth() -2, 1).setHours(23, 59, 59, 999));
            $scope.vm.toDate2 = new Date(new Date(date.getFullYear(), date.getMonth(), 1).setHours(23, 59, 59, 999));
            $scope.get()
        };
        ...
        var filterData = function () {
            ...
            var fromDate = new Date($scope.vm.fromDate.getFullYear(), $scope.vm.fromDate.getMonth(), 1);
            var yearDiff = $scope.vm.fromDate2.getFullYear() - $scope.vm.fromDate.getFullYear();
            $scope.vm.failureStats = [];
            while (new Date(fromDate).getTime() <= new Date($scope.vm.toDate).getTime()) {
                $scope.vm.failureStats.push({ name: monthNames[fromDate.getMonth()], fromDate1: angular.copy(fromDate), fromDate2: new Date(fromDate.getFullYear() + yearDiff, fromDate.getMonth(), 1) });
        ...
        var setFailureStats = function () {
            ...
            $scope.vm.totalchange = 0;
            $scope.vm.totalnchange = 0;
            $scope.vm.totalCCount1 = 0;
            $scope.vm.totalNCount1 = 0;
            $scope.vm.totalCCount2 = 0;
            $scope.vm.totalNCount2 = 0;
            if ($scope.vm.failureStats != null && $scope.vm.failureStats.length > 0) {
                $scope.vm.failureStats.forEach(function (item) {
                    var count1 = $scope.vm.failures1.filter(function (failure1) { return failure1.stimeOfOccurance.getFullYear() == item.fromDate1.getFullYear() && failure1.stimeOfOccurance.getMonth() == item.fromDate1.getMonth() && failure1.failureChargeable }).length;
                    var ncount1 = $scope.vm.failures1.filter(function (failure1) { return failure1.stimeOfOccurance.getFullYear() == item.fromDate1.getFullYear() && failure1.stimeOfOccurance.getMonth() == item.fromDate1.getMonth() && !failure1.failureChargeable}).length;
                    var count2 = $scope.vm.failures2.filter(function (failure2) { return failure2.stimeOfOccurance.getFullYear() == item.fromDate2.getFullYear() && failure2.stimeOfOccurance.getMonth() == item.fromDate2.getMonth() && failure2.failureChargeable}).length;
                    var ncount2 = $scope.vm.failures2.filter(function (failure2) { return failure2.stimeOfOccurance.getFullYear() == item.fromDate2.getFullYear() && failure2.stimeOfOccurance.getMonth() == item.fromDate2.getMonth() && !failure2.failureChargeable}).length;
                    $scope.vm.totalcount1 += (count1 + ncount1);
                    $scope.vm.totalcount2 += (count2 + ncount2);
                    $scope.vm.totalCCount1 += count1;
                    $scope.vm.totalNCount1 += ncount1;
                    $scope.vm.totalCCount2 += count2;
                    $scope.vm.totalNCount2 += ncount2;
                    item.count1 = count1;
                    item.count2 = count2;
                    item.ncount1 = ncount1;
                    item.ncount2 = ncount2;
                    item.tcount1 = (count1 + ncount1);
                    item.tcount2 = (count2 + ncount2);
                    var change = count1 > 0 ? ((count2 - count1) / count1) * 100 : 0;
                    var nchange = ncount1 > 0 ? ((ncount2 - ncount1) / ncount1) * 100 : 0;
                    var tchange = item.tcount1 > 0 ? ((item.tcount2 - item.tcount1) / item.tcount1) * 100 : 0;
                    item.change = change.toFixed(1);
                    item.nchange = nchange.toFixed(1);
                    item.tchange = tchange.toFixed(1);
                });

                var tchange = $scope.vm.totalCCount1 > 0 ? (($scope.vm.totalCCount2 - $scope.vm.totalCCount1) / $scope.vm.totalCCount1) * 100 : 0;
                $scope.vm.totalCchange = tchange.toFixed(1);
                var tnchange = $scope.vm.totalNCount1 > 0 ? (($scope.vm.totalNCount2 - $scope.vm.totalNCount1) / $scope.vm.totalNCount1) * 100 : 0;
                $scope.vm.totalnchange = tnchange.toFixed(1);
                var change = $scope.vm.totalcount1 > 0 ? (($scope.vm.totalcount2 - $scope.vm.totalcount1) / $scope.vm.totalcount1) * 100 : 0;
                $scope.vm.totalchange = change.toFixed(1);
            }
        };
