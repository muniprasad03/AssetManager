...
appRoot.controller('sectionGearIncidentsReportController', [
    ...
    function ($scope, $q, $state, $stateParams, $location, $filter, $http, $uibModal, ReportService, UserService, NotificationService, SharedModel, AppSettings) {
        ...
        var filterData = function () {
            ...
            $scope.vm.reportGears = $scope.vm.gearFaultFilter.id == 0 ? $scope.vm.gearFaults : $scope.vm.gearFaultFilter.items;
        ...
        var setFailureStats = function () {
            ...
            if ($scope.vm.failureStats != null && $scope.vm.failureStats.length > 0) {
                $scope.vm.failureStats.forEach(function (item) {
                    var cfailures1 = $scope.vm.failures1.filter(function (failure1) { return failure1.sectionId == item.id });
                    //var ncfailures1 = $scope.vm.failures1.filter(function (failure1) { return failure1.sectionId == item.id && !failure1.failureChargeable });
                    var cfailures2 = $scope.vm.failures2.filter(function (failure2) { return failure2.sectionId == item.id });
                    //var ncfailures2 = $scope.vm.failures2.filter(function (failure2) { return failure2.sectionId == item.id && !failure2.failureChargeable });
                    $scope.vm.reportGears.forEach(function (gear) {
                        item[gear.id + 'fault1'] = cfailures1.filter(function (f) { return f.gearFaultId == gear.id }).length;
                        item[gear.id + 'fault2'] = cfailures2.filter(function (f) { return f.gearFaultId == gear.id }).length;
                        $scope.vm.totalStats[gear.id + 'total1'] = item[gear.id + 'fault1'] + ($scope.vm.totalStats[gear.id + 'total1'] || 0);
                        $scope.vm.totalStats[gear.id + 'total2'] = item[gear.id + 'fault2'] + ($scope.vm.totalStats[gear.id + 'total2'] || 0);
                    });

                    item['total1'] = cfailures1.length;
                    item['total2'] = cfailures2.length;
                    var change = cfailures1.length > 0 ? ((cfailures2.length - cfailures1.length) / cfailures1.length) * 100 : 0;
                    item.change = change.toFixed(1);
                });

                var tchange = $scope.vm.failures1.length > 0 ? (($scope.vm.failures2.length - $scope.vm.failures1.length) / $scope.vm.failures1.length) * 100 : 0;
                $scope.vm.change = tchange.toFixed(1);
                //var tnchange = $scope.vm.totalcount2 > 0 ? (($scope.vm.totalcount2 - $scope.vm.totalcount1) / $scope.vm.totalcount2) * 100 : 0;
