...
namespace AssetManager.Services
{
    public class PointService : ServiceBase, IWorkService
    {
        ...
        public PointService(IRequestContext requestContext, XWorkDB db)
            : base(requestContext)
        {
            this.DB = db;
        }
        ...
        public List<AssetManager.Models.Pointer> Get()
        {
            ...
            {
                return this.DB.Fetch<AssetManager.Data.Model.Work>("Select * from Work").MapCollectionTo<Data.Model.Work, AssetManager.Models.Pointer>().ToList();
            }
            else
            {
                return this.DB.Fetch<Data.Model.Work>("Select * from Work Where UserId = @0", this.Context.User.Id).MapCollectionTo<Data.Model.Work, AssetManager.Models.Pointer>().ToList();
            }
        }

        public AssetManager.Models.Pointer Get(int id)
        {
            return this.DB.SingleOrDefault<Data.Model.Work>("Where Id = @0", id).MapTo<AssetManager.Models.Pointer>();
        }
