...
appRoot.controller('sectionIncidentsReportController', [
    ...
    function ($scope, $q, $state, $stateParams, $location, $filter, $http, $uibModal, ReportService, UserService, NotificationService, SharedModel, AppSettings) {
        ...
        var filterData = function () {
            ...
            $scope.vm.failureMonths = [];
            while (new Date(fromDate).getTime() <= new Date($scope.vm.toDate).getTime()) {
                $scope.vm.failureMonths.push({ name: monthNames[fromDate.getMonth()], fromDate1: angular.copy(fromDate), fromDate2: new Date(fromDate.getFullYear() + yearDiff, fromDate.getMonth(), 1) });
                fromDate = new Date(fromDate.getFullYear(), fromDate.getMonth() + 1, 1);
            }

            $scope.vm.failureStats = $scope.vm.sectionFilter.id == 0 ? $scope.vm.sections : $scope.vm.sectionFilter.items;
        ...
        var setFailureStats = function () {
            ...
            $scope.vm.totalCchange = 0;
            $scope.vm.totalnchange = 0;
            $scope.vm.changeTotal = 0;
            ...
            if ($scope.vm.failureStats != null && $scope.vm.failureStats.length > 0) {
                $scope.vm.failureStats.forEach(function (item) {
                    var count1 = $scope.vm.failures1.filter(function (failure1) { return failure1.sectionId == item.id && failure1.failureChargeable }).length;
                    var ncount1 = $scope.vm.failures1.filter(function (failure1) { return failure1.sectionId == item.id && !failure1.failureChargeable }).length;
                    var count2 = $scope.vm.failures2.filter(function (failure2) { return failure2.sectionId == item.id && failure2.failureChargeable }).length;
                    var ncount2 = $scope.vm.failures2.filter(function (failure2) { return failure2.sectionId == item.id && !failure2.failureChargeable }).length;
                    ...
                    item.cCount = (item.tcount2 - item.tcount1)
                ...
                var tcchange = $scope.vm.totalCCount1 > 0 ? (($scope.vm.totalCCount2 - $scope.vm.totalCCount1) / $scope.vm.totalCCount1) * 100 : 0;
                $scope.vm.totalCchange = tcchange.toFixed(1);
                ...
                var tchange = $scope.vm.totalcount1 > 0 ? (($scope.vm.totalcount2 - $scope.vm.totalcount1) / $scope.vm.totalcount1) * 100 : 0;
                $scope.vm.changeTotal = tchange.toFixed(1);
            }
        };
