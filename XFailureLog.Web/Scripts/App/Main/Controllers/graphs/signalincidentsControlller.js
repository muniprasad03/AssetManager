...
appRoot.controller('signalIncidentsReportController', [
    ...
    function ($scope, $q, $state, $stateParams, $location, $filter, $http, $uibModal, ReportService, UserService, NotificationService, SharedModel, AppSettings) {
        ...
        var setFailureStats = function () {
            ...
            if ($scope.vm.failureStats.length > 0) {
                $scope.vm.failureStats.forEach(function (item) {
                    var count1 = $scope.vm.failures1.filter(function (failure1) { return failure1.stimeOfOccurance.getFullYear() == item.fromDate1.getFullYear() && failure1.stimeOfOccurance.getMonth() == item.fromDate1.getMonth() }).length;
                    var count2 = $scope.vm.failures2.filter(function (failure2) { return failure2.stimeOfOccurance.getFullYear() == item.fromDate2.getFullYear() && failure2.stimeOfOccurance.getMonth() == item.fromDate2.getMonth() }).length;
                    $scope.vm.totalcount1 += count1;
                    $scope.vm.totalcount2 += count2;
                    item.count1 = count1;
                    item.count2 = count2;
                    var change = count2 > 0 ? ((count2 - count1) / count2) * 100 : 0;
                    item.change = change.toFixed(1);
                });

                var tchange = $scope.vm.totalcount2 > 0 ? (($scope.vm.totalcount2 - $scope.vm.totalcount1) / $scope.vm.totalcount2) * 100 : 0;
                $scope.vm.totalchange = tchange.toFixed(1);
            }
        };
